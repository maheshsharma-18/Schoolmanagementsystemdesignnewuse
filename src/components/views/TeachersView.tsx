  ...
  const fetchTeachers = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-2f5f0fc8/teachers`,
        {
          headers: {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">
              {language === 'en' ? 'Teachers Management' : 'ఉపాధ్యాయుల నిర్వహణ'}
            </h1>
            <p className="text-muted-foreground">
              {language === 'en' ? 'Manage all teaching staff and their performance' : 'అన్ని బోధనా సిబ్బంది మరియు వారి పనితీరును నిర్వహించండి'}
            </p>
          </div>
        </div>
