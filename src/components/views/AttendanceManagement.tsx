  ...
  const fetchAttendanceData = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-2f5f0fc8/attendance/overview`,
        {
          headers: {
...
function ClassAttendanceDetail({ classData, onBack }: ClassAttendanceDetailProps) {
  ...
  const fetchClassStudents = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-2f5f0fc8/attendance/class/${classData.id}`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
          },
